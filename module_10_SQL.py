...
class OutputPath:
    def __init__(self):
        success_path = 'module_10/news.txt'
...
class DBConnector:
    ...
    def insert(self, table_name, body, extension_1, extension_2, param):
        insert = f'''
                INSERT or REPLACE INTO {table_name} ({body}, {extension_1}, {extension_2})
                VALUES
                (?, ?, ?)
                '''
    ...
    def split_content(content):
        ...
        for block in content.split('\n\n'):
            ...
            for part in block.split('\n'):
                news_block.append(part)
            if len(news_block) > 1:
                for extension in news_block[2].split(', '):
                    news_block.insert(2, extension)
                    del news_block[4:]

                news_block[0] = re.sub(r'[^\w\s]', '', news_block[0])
                news_block[2] = news_block[2].replace(' days left', '')
                news_block[2] = news_block[2].replace('% probability', '')
                news_block[3] = news_block[3].replace('Actual until: ', '')
                news_block[3] = news_block[3].replace('Zodiac sign: ', '')
                blocks.append(news_block)
        return blocks

    def db_save_records(self, publication):
        db_path = OutputPath()
        connector = DBConnector(db_path.get_db_file())
        ...
        connector.execute('''
            ...
            AdvBody TEXT NOT NULL,
            ActualUntil TEXT NOT NULL,
        ...
        connector.execute('''
            ...
            HorId INTEGER PRIMARY KEY AUTOINCREMENT,
            HorBody TEXT NOT NULL,
        ...
        content = publication.get_content()
        for i in self.split_content(content):
            params = (i[1], i[3], i[2])
            if i[0] == 'News':
                connector.insert('News', 'NewsBody', 'City', 'CurrentDate', params)
            elif i[0] == 'Advertising':
                connector.insert('Advertising', 'AdvBody', 'ActualUntil', 'DaysLeft', params)
            elif i[0] == 'Horoscope':
                connector.insert('Horoscope', 'HorBody', 'ZodiacSign', 'Probability', params)
        connector.conn.commit()
        # warnings.filterwarnings("ignore", category=DeprecationWarning)
        # print(connector.fetch_all('select * from News'))
        # print(connector.fetch_all('select * from Advertising'))
        # print(connector.fetch_all('select * from Horoscope'))
    ...
    def run():
        try:
            ...
            if isinstance(newsfeed_publication, TextPublication) is True:
                ...
                db_saving = DBPublisher()
                db_saving.db_save_records(newsfeed_publication)
